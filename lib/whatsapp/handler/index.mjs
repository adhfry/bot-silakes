...
import {
  handleTesNarkoba,
  handlePemeriksaanSebelumnya,
  handleKonfirmasiDaftarPemeriksaan,
} from "./tesNarkoba.mjs";
...
export async function handleReplyChatMessage(message) {
  ...
  if (
    ...
    (lowerQuotedText.includes(
      "apakah hari ini anda ingin melakukan pemeriksaan"
    ) ||
      lowerQuotedText.includes("apakah anda ingin melakukan pemeriksaan"))
  ) {
    return await handleKonfirmasiDaftarPemeriksaan(message);
  }
  if (
    lowerQuotedText.startsWith(
      "⚠️ anda memiliki pemeriksaan sebelumnya yang belum selesai."
    )
  ) {
    console.log("kesini");
    return await handlePemeriksaanSebelumnya(message);
  }
}
