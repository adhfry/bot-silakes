...
export async function handleScanKTP(message) {
  ...
  // teks berupa #ai scan ktp, 081233107475
  // dapatkan nomor hp
  const noHp = text.split(",")[1]?.trim();
  if ((noHp && !noHp.match(/^(08|628)[0-9]+$/)) || !noHp) {
    return await replyChatWA(
      message,
      "❌ *Nomor HP tidak valid.*\n" +
        "Pastikan nomor HP diawali dengan 08 atau 628.\n\nContoh: #ai scan ktp, 081234567890"
    );
  }
  ...
  if (!message.hasMedia) {
    return await replyChatWA(
      ...
      "❌ *Anda belum mengirim foto KTP.*\n" +
        "Gunakan: `#ai scan ktp, [noHp]` diikuti dengan lampiran foto KTP."
    );
  }
