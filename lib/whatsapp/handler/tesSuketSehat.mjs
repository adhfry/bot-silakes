...
export async function handleTesSuketSehat(message) {
  const from = message.from;
  const phone = from.replace("@c.us", "");
  const textBody = message.body.trim();
  const lowerTextBody = textBody.toLowerCase();
  const layananPasien = textBody.split("#ai tes ")[1];
  ...
  if (existing) {
    return await replyChatWA(
      message,
      `⚠️ Anda memiliki pemeriksaan (${existing.layanan}) sebelumnya yang belum selesai.\n\n` +
        `1️⃣ Lanjutkan pemeriksaan saat ini (${layananPasien}) \n` +
  ...
  await replyChatWA(
    message,
    `Apakah hari ini Anda ingin melakukan pemeriksaan *Surat Keterangan Sehat* seharga *Rp 30.000* pada ${waktu}?\n\n` +
 ...
 */
export async function handlePemeriksaanSebelumnyaSuketSehat(msg) {
  const from = msg.from;
  ...
  if (!current) {
    return await replyChatWA(
      ...
      "❌ *Tidak ada proses pemeriksaan yang ditemukan.*\n" +
        "Silakan mulai dengan perintah `#ai tes suket sehat`"
    );
  }
  const waktu = getTimeNow();
  switch (lowerText) {
    ...
    case "lanjutkan pemeriksaan sebelumnya":
      await replyChatWA(
        ...
        "✅ Pemeriksaan *sebelumnya* akan dilanjutkan.\n\n" +
          "Mohon konfirmasi kembali:\n" +
          `Apakah hari ini Anda ingin melakukan pemeriksaan *${current.layanan}* pada ${waktu}?\n\n` +
          `Reply dengan: *Y / YA / IYA*`
      );
      break;
...
export async function handleKonfirmasiDaftarPemeriksaanSuketSehat(msg) {
  const from = msg.from;
  const text = msg.body.trim();
  const lowerText = text.toLowerCase();
  const phone = from.replace("@c.us", "");
